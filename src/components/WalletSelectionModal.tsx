...
export const WalletSelectionModal: React.FC<WalletSelectionModalProps> = ({
	...
	const detectWallets = (): number => {
		try {
			...
			const wallets: CardanoWallet[] = CARDANO_WALLETS_INFO.map(
				(walletInfo) => {
					const allPossibleIds = [
						walletInfo.name,
						...walletInfo.alternativeIds,
					];
					const matchingId = allPossibleIds.find((id) =>
						installedWalletIds.includes(id)
					);
					...
					const icon =
						...
						`https://ui-avatars.com/api/?name=${walletInfo.displayName}&background=random`;
	...
	const handleWalletConnect = async (walletName: string) => {
		...
		try {
			...
			console.log(
				`🔌 Connecting to wallet: ${walletInfo?.displayName} (ID: ${walletIdToConnect})`
			);

			setCurrentStep("Connecting to wallet...");

			// Use Mesh SDK's connect method
			await connect(walletIdToConnect as any);

			// Get wallet instance from BrowserWallet for additional operations
			...
			if (!userAddress) {
				throw new Error(
					"No address found in wallet. Please make sure your wallet has at least one address."
				);
			}
			...
			if (error.message?.includes("User declined")) {
				...
				errorMessage =
					"Cannot connect to authentication server. Check if backend is running.";
				...
				{isConnecting ? (
					<div className="flex flex-col items-center justify-center py-8 space-y-4">
						...
						<div className="text-center space-y-1">
							<p className="text-sm font-medium text-primary">
								{connectingWallet
									? `Connecting to ${
											availableWallets.find((w) => w.name === connectingWallet)
												?.displayName
									  }...`
									: "Connecting..."}
							</p>
					...
					<>
						<div className="text-center mb-6">
							<h2 id="wallet-modal-title" className="text-xl font-bold mb-1">
								Connect Wallet
							</h2>
							...
							<div className="space-y-2">
								...
								{notInstalledWallets.map((wallet) => (
									...
									>
										...
										<span className="font-medium flex-1 text-left text-gray-500 group-hover:text-gray-300 transition-colors">
											{wallet.displayName}
										</span>
										<ExternalLink
											size={14}
