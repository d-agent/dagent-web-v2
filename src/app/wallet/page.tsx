...
export default function WalletPage() {
	const { getStakeByAddress } = useStake();
	...
	console.log("Wallet address from hook:", address);
		...
		if (wallet?.getBalance) {
			wallet
				...
				.then((assets) => {
					...
					setWalletStats((prev) => ({ ...prev, balance: total }));
				})
				.catch((error) => {
	...
	}, [getStakeByAddress, wallet, connected]);
	...
	const handleStakeSuccess = (amount: number, txHash: string) => {
		setWalletStats((prev) => ({
			...prev,
	...
	const handleWithdrawSuccess = (amount: number, txHash: string) => {
		setWalletStats((prev) => ({
			...prev,
				...
				<div className="lg:col-span-2 relative h-[300px] rounded-3xl overflow-hidden p-10 flex flex-col justify-between border border-white/10 shadow-2xl group">
					...
					<div className="relative z-10">
						...
						<div className="text-6xl font-bold font-mono tracking-tighter text-white">
							{/* {wallet.getBalance().toLocaleString()}{" "} */}
							<span className="text-3xl text-primary">ADA</span>
						</div>
				...
				<div className="bg-surface border border-white/10 rounded-3xl p-8 flex flex-col justify-between">
						...
						<div className="space-y-6">
							<div>
								...
								<div className="text-2xl font-mono font-bold">
									{walletStats.staked.toLocaleString()} ADA
								</div>
							...
							<div>
								...
								<div className="text-2xl font-mono font-bold text-primary">
									{walletStats.earnings.toLocaleString()} ADA
								</div>
							</div>
						</div>
					</div>
					<button
						onClick={() => {
							setWalletStats((prev) => ({
								...prev,
								balance: prev.balance + prev.earnings,
								earnings: 0,
							}));
							setSuccessType("withdraw");
							setSuccessAmount(walletStats.earnings);
							setSuccessTxHash("claim-rewards-" + Date.now());
							setShowSuccessModal(true);
						}}
							...
							>
								...
								<div className="text-right">
									<div className="font-mono font-bold">
										{stake.amount.toLocaleString()} ADA
									</div>
								...
								<div className="flex justify-between items-start">
									<div>
										...
										<div className="text-xs text-gray-500 flex items-center gap-2 mt-1">
											...
											<span className="font-mono text-[10px] text-gray-600 truncate w-24">
												{(tx as any).hash}
											</span>
										</div>
									</div>
